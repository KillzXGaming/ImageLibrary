...
namespace ImageLibrary
{
    ...
    public class AstcFile
    {
        ...
        public AstcFile(Stream stream) => Read(new BinaryReader(stream));
        public AstcFile(string filePath)
        {
            using (var fs = new FileStream(filePath, FileMode.Open, FileAccess.Read))
                Read(new BinaryReader(fs));
        }
        public void Save(Stream stream) => Write(new BinaryWriter(stream));
        ...
        public Astc GetEncoder()
        {
            return new Astc(GetFormat());
        }
    }
}
