...
namespace ImageLibrary
{
    ...
    public class DdsFile
    {
        ...
        public enum D3D10_RESOURCE_MISC2_FLAG
        {
        ...
        public ImageFormat Encoder => new ImageFormat(Format);
        ...
        public uint ArrayCount
        {
            get
            {
                if (IsDX10 && IsCubeMap) return Math.Max(Dx10Header.ArrayCount, 1) * 6;
                if (IsDX10) return Math.Max(Dx10Header.ArrayCount, 1);
        ...
        public DdsFile(bool isDxt10 = false) {
            ...
            Dx10Header.ResourceDim = (uint)D3D10_RESOURCE_DIMENSION.D3D10_RESOURCE_DIMENSION_TEXTURE2D;
        ...
        public GenericTextureBase ToGeneric()
        {
            TextureType GetType()
            {
                if (IsCubeMap && this.ArrayCount > 6)
                    return TextureType.TextureCubeArray;
                if (IsCubeMap)
                    return TextureType.TextureCube;
                if (Dx10Header.ResourceDim == (uint)D3D10_RESOURCE_DIMENSION.D3D10_RESOURCE_DIMENSION_TEXTURE3D)
                    return TextureType.Texture3D;

                return TextureType.Texture2D;
            }

            // Supported formats for rencoding
            List<IImageFormat> supportedFormats = new List<IImageFormat>();
            foreach (DXGI_FORMAT format in Enum.GetValues(typeof(DXGI_FORMAT)))
            {
                if (ImageFormat.IsEncoderSupported(format))
            ...
            var platform = new PlatformSwizzleBase();
            // Todo detect other platforms if DDS was modified in a way that can detect it

            return new GenericTextureBase()
            {
                Width = this.MainHeader.Width,
                Height = this.MainHeader.Height,
                MipCount = this.MainHeader.MipCount,
                Depth = this.MainHeader.Depth,
                ArrayCount = this.MainHeader.Depth > 1 ? this.MainHeader.Depth : this.ArrayCount,
                Data = this.ImageData,
                ImageFormat = this.Encoder,
                SupportedFormats = supportedFormats,
                Type = GetType(),
                PlatformSwizzle = platform,
                RedComponent = TextureChannelType.Red,
        ...
        public void SetFlags(DXGI_FORMAT format, bool ForceDX10 = false, bool isCubeMap = false)
        {
            ...
            if (IsDX10 || ForceDX10)
            {
                ...
                if (isCubeMap)
                {
                    ...
                    Dx10Header.MiscFlags1 = (uint)D3D10_RESOURCE_MISC_FLAG.DDS_RESOURCE_MISC_TEXTURECUBE;
                }
                return;
            }
        }
        ...
        private void ConvertDXGIFormatToFlags(DXGI_FORMAT format)
        {
            switch (Format)
            {
                ...
                case DXGI_FORMAT.DXGI_FORMAT_A8_UNORM:
                    PfHeader.Flags = (uint)(DDPF.ALPHA);
                    PfHeader.RgbBitCount = 0x8;
                    PfHeader.RBitMask = (uint)0;
                    PfHeader.GBitMask = (uint)0;
                    PfHeader.BBitMask = (uint)0;
                    PfHeader.ABitMask = (uint)L8_MASKS[0];
                    break;
        ...
        public uint CalculateSize()
        {
            ...
            for (int arrayLevel = 0; arrayLevel < ArrayCount; arrayLevel++)
            {
                for (int mipLevel = 0; mipLevel < this.MainHeader.MipCount; mipLevel++)
                {
                    ...
                    uint imageSize = formatInfo.GetSize((uint)mipWidth, (uint)mipHeight); 
                    size += imageSize;
                }
            }
            return size;
