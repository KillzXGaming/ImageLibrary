...
namespace ImageLibrary.Formats.Encoders
{
    ...
    public class Rgba : ImageEncoder
    {
        ...
        public uint BitsPerPixel => (uint)(R + G + B + A);
                ...
                for (uint x = 0; x < width; x++)
                {
                    ...
                    for (int i = 0; i < ChannelOrder.Length; i++)
                    {
                        switch (ChannelOrder[i])
                        {
                            ...
                            case 'A': output[pixelIndex + i] = (byte)(A == 0 ? 255 : a); break;
        ...
        {
            if (bitCount == 0) return 0; //bit unused
        ...
        public byte[] Encode(byte[] data, uint width, uint height)
        {
            ...
            for (uint y = 0; y < height; y++)
            {
                for (uint x = 0; x < width; x++)
                {
                    ...
                    uint pixelData = 0;
                    pixelData |= PackChannel(r, 0, R);
                    pixelData |= PackChannel(g, R, G);
                    pixelData |= PackChannel(b, R + G, B);
                    pixelData |= PackChannel(a, R + G + B, A);
        ...
        private uint PackChannel(byte value, int bitOffset, int bitCount)
        {
            ...
            {
                int maxPositiveValue = (1 << bitCount - 1) - 1; // Max positive value for signed int
                int signedValue = value * (2 * maxPositiveValue) / 255 - maxPositiveValue;
        ...
        {
            if (ChannelOrder.Length <= channelIndex) return 0;
