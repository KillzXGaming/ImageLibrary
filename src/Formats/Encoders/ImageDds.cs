...
namespace ImageLibrary.Formats.Encoders
{
    public class ImageDds
    {
            ...
            ImageFormat format, Quality quality = Quality.Normal)
        {
            ...
            {
                if (encodedDataPtr != nint.Zero)
                    ImageDdsNative.free_encoded_data(encodedDataPtr);
            }
        }

        static byte[] ImageToMultiple4(byte[] data, int width, int height)
        {
